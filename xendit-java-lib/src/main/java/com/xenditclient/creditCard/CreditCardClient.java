...
public class CreditCardClient {
    ...
    public CreditCardCharge createAuthorization(Map<String, Object> params)
            throws XenditException {
    ...
    public CreditCardReverseAuth reverseAuthorization(
        ...
        String url =
                String.format(
                        "%s%s%s%s",Xendit.Opt.getXenditURL(), "/credit_card_charges/", chargeId, "/auth_reversal");
        Map<String, Object> params = new HashMap<>();
        params.put("external_id", externalId);
    ...
    public CreditCardCharge captureCharge(String chargeId, Number amount)
            throws XenditException {
        return captureCharge(new HashMap<>(), chargeId, amount);
    }

    public CreditCardCharge captureCharge(
            Map<String, String> headers, String chargeId, Number amount) throws XenditException {
        Map<String, Object> params = new HashMap<>();
        params.put("amount", amount);
        String url =
                String.format("%s%s%s%s", Xendit.Opt.getXenditURL(), "/credit_card_charges/", chargeId, "/capture");
    ...
    public CreditCardRefund createRefund(
        ...
        params.put("external_id", externalId);
        String url =
                String.format("%s%s%s%s", Xendit.Opt.getXenditURL(), "/credit_card_charges/", id, "/refunds");
