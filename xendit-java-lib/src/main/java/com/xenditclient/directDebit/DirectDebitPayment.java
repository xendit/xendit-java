  ...
  protected static DirectDebitPaymentClient getClient() {
    if (isApiKeyExist()) {
      if (directDebitPaymentClient == null
        ...
        return directDebitPaymentClient =
                new DirectDebitPaymentClient(Xendit.Opt.setApiKey(Xendit.apiKey), Xendit.getRequestClient());
      }
    } else {
      if (directDebitPaymentClient == null
              || !directDebitPaymentClient.getOpt().getApiKey().trim().equals(Xendit.Opt.getApiKey().trim())) {
        return directDebitPaymentClient = new DirectDebitPaymentClient(Xendit.Opt, Xendit.getRequestClient());
      }
    }
    return directDebitPaymentClient;
  }
