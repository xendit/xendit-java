...
public class InvoiceClient {
    ...
    public Invoice[] getAll(Map<String, String> headers, Map<String, Object> params) throws XenditException {
        ...
        String url = String.format("%s%s%s", com.xendit.Xendit.getUrl(), "/v2/invoices?", parameters.toString());
        return this.requestClient.request(
                RequestResource.Method.GET, url, headers, null, opt.getApiKey(), Invoice[].class);
    }
    ...
    public Invoice expire(Map<String, String> headers, String id) throws XenditException {
        String url = String.format("%s%s%s%s", com.xendit.Xendit.getUrl(), "/invoices/", id, "/expire!");
        return this.requestClient.request(
