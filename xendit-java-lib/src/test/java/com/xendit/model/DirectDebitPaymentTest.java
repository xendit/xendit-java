...
public class DirectDebitPaymentTest {
  ...
  private static String URL = String.format("%s%s", Xendit.getUrl(), "/direct_debits");
  private static String VALIDATE_URL =
      String.format(
          "%s%s%s%s",
          Xendit.getUrl(), "/direct_debits/", DIRECT_DEBIT_PAYMENT_ID, "/validate_otp/");
  private static String GET_BY_ID_URL =
      String.format("%s%s%s%s", Xendit.getUrl(), "/direct_debits/", DIRECT_DEBIT_PAYMENT_ID, "/");
  ...
  public void createDirectDebitPayment_Success_IfParamsIsValid() throws XenditException {
    ...
    DirectDebitPayment directDebitPayment =
        DirectDebitPayment.createDirectDebitPayment(PARAMS, IDEMPOTENCY_KEY);
  ...
  public void createDirectDebitPayment_Success_WithHeaderProvided() throws XenditException {
    ...
    DirectDebitPayment directDebitPayment =
        DirectDebitPayment.createDirectDebitPayment(HEADERS, PARAMS, IDEMPOTENCY_KEY);
  ...
  public void validateOTP_Success_IfMethodCalledCorrectly() throws XenditException {
    ...
    when(Xendit.requestClient.request(
            RequestResource.Method.POST, VALIDATE_URL, HEADERS, PARAMS, DirectDebitPayment.class))
        .thenReturn(VALID_VALIDATED_DIRECT_DEBIT_PAYMENT);

    DirectDebitPayment directDebitPayment =
        DirectDebitPayment.validateOTP(DIRECT_DEBIT_PAYMENT_ID, OTP_CODE);
  ...
  public void validateOTP_Success_IfParamsIsValid() throws XenditException {
    ...
    DirectDebitPayment directDebitPayment =
        DirectDebitPayment.validateOTP(DIRECT_DEBIT_PAYMENT_ID, PARAMS);
  ...
  public void validateOTP_Success_WithHeaderProvided() throws XenditException {
    ...
    DirectDebitPayment directDebitPayment =
        DirectDebitPayment.validateOTP(DIRECT_DEBIT_PAYMENT_ID, HEADERS, PARAMS);
