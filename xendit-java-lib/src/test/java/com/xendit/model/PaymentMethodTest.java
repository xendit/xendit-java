...
public class PaymentMethodTest {
  private static String URL = String.format("%s%s", Xendit.getUrl(), "/payment_methods");
  ...
  public void createPaymentMethod_Success_WithHeaderProvided() throws XenditException {
    ...
    PaymentMethod paymentMethod = PaymentMethod.createPaymentMethod(HEADERS, PARAMS);
  ...
  public void createPaymentMethod_ThrowsException_IfParamsIsInvalid() throws XenditException {
    ...
    when(Xendit.requestClient.request(
            RequestResource.Method.POST, URL, new HashMap<>(), PARAMS, PaymentMethod.class))
        .thenThrow(new XenditException("Payment method type is invalid"));

    PaymentMethod.createPaymentMethod(PARAMS);
  }
  ...
  public void getPaymentMethods_Success_IfCustomerIdIsAvailable() throws XenditException {
    ...
    when(Xendit.requestClient.request(RequestResource.Method.GET, url, null, PaymentMethod[].class))
        .thenReturn(PAYMENT_METHOD_ARRAY);

    PaymentMethod[] paymentMethods = PaymentMethod.getPaymentMethodsByCustomerId(CUSTOMER_ID);
  ...
  public void getPaymentMethods_ThrowsException_IfCustomerIdIsNotAvailable()
    ...
    PaymentMethod.getPaymentMethodsByCustomerId(NOT_AVAILABLE_CUSTOMER_ID);
  }
}
